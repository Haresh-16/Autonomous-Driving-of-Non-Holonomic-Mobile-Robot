...
class ParticleFilter():
  ...
  def expected_pose(self):
    # YOUR CODE HERE
    # Calculate the weighted average of the particles
    x_weights = self.weights * self.particles[:, 0]
    y_weights = self.weights * self.particles[:, 1]
    # theta_weights = self.weights * self.particles[:, 2]

    # Calculate the expected pose
    x_expected = np.sum(x_weights) / np.sum(self.weights)
    y_expected = np.sum(y_weights) / np.sum(self.weights)
    
    sin_sum = np.sum(self.weights*np.sin(self.particles[:,2])) 
    cos_sum = np.sum(self.weights*np.cos(self.particles[:,2])) 
    
    theta_expected = np.arctan2(sin_sum,cos_sum)

    return np.array([x_expected, y_expected, theta_expected])
