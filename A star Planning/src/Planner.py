...
bag = rosbag.Bag('/home/robot/mushr_ws/src/planner/final_bag.bag','w')
...
class PlannerNode(object):
  ...
  def record_bag(self,pose_array):
    bag.write('/pose_array_topic',pose_array)
  ...
  def publish_plan(self, plan):
    ...
    #self.record_bag(pa)
    with rosbag.Bag('/home/robot/mushr_ws/src/planner/path_to_bag_file.bag', 'w') as bag:
      bag.write('/pose_array_topic', pa)
      print("Bag File writed. \n")
...
def get_waypoint(path):
  ...
  dir_path = '/home/robot/mushr_ws/src/planner/waypoints2' #str(Path(dir_path).parent)
...
if __name__ == '__main__':
  ...
  csv_start = '/start.csv'
  csv_good = '/good_waypoints.csv'
  csv_bad = '/bad_waypoints.csv'
  print("[Planner Node] Getting map from service...")
  rospy.wait_for_service(map_service_name)
  ...
  while not rospy.is_shutdown():
    ...
    # Read the plan from the CSV file
    csv_file_path = "/home/robot/mushr_ws/src/planner/plan_file_team7.csv"
    pn.cur_plan = np.loadtxt(csv_file_path, delimiter=',', skiprows=1)
    print('Read plan from CSV:', pn.cur_plan)
    pn.publish_plan(pn.cur_plan)
    pn.publish_waypoints_viz()
    print('Planned path published to topic planner_node/car_plan')
