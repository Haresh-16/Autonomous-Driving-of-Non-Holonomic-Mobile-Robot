...
def world_to_map(pose, map_info):
    ...
    config = [0.0, 0.0]
    # translation - Translates the x and y components of the pose based on the map's origin and scale.
    config[0] = (1.0 / float(scale)) * (pose[0] - map_info.origin.position.x)
    config[1] = (1.0 / float(scale)) * (pose[1] - map_info.origin.position.y)

    # rotation - Rotates the pose based on the calculated angle
...
def map_to_world(pose, map_info):
    ...
    # config = np.array([pose[0], pose[1], pose[2]])
    # rotation
    c, s = np.cos(angle), np.sin(angle)
    # we need to store the x coordinates since they will be overwritten
    temp = np.copy(config[0])
